...
func HasPoolLicenseError(poolname string, searchtext string) (bool, error) {
	namespace := common.NSDefault
	options := metaV1.ListOptions{
		TypeMeta: metaV1.TypeMeta{Kind: "DiskPool"},
	}
	eventList, listerr := GetEvents(common.NSMayastor(), options)
	if listerr != nil {
		err := fmt.Errorf("failed to get pool events, namespace: %s, error: %v", namespace, listerr)
		return false, err
	}
	for _, event := range eventList.Items {
		if strings.HasPrefix(event.Name, poolname) && event.Type == "Critical" && strings.Contains(event.Message, searchtext) {
			logf.Log.Info("Found pool event", "message", event.Message, "search text", searchtext, "pool", poolname)
			return true, nil
		}
	}
	return false, nil
...
// Expects all pools to go to errored state and have a license failure event
func CreateConfiguredPoolsAndFail(timeoutsecs int) error {
	err := CreateConfiguredPools()
	if err != nil {
		return err
	}
	const sleepTime = 5
	var all_errored bool

	for ix := 1; ix < timeoutsecs/sleepTime; ix++ {
		pool_list, err := custom_resources.ListMsPools()
		if err != nil {
			break
		}
		...
		for _, pool := range pool_list {
			cr_state := pool.GetCRStatus()
			event_found, listerr := HasPoolLicenseError(pool.GetName(), common.PoolLicenseFailureEventText)
			if listerr != nil {
				err = fmt.Errorf("failed to list pool events: error: %s", listerr.Error())
				break
			}
			if cr_state != "Creating" || !event_found {
				all_errored = false
				break
			}
		}
		if all_errored || err != nil {
			break
		}
		time.Sleep(sleepTime * time.Second)
	}
	// _ = DeleteConfiguredPools()
	...
	if !all_errored {
		return fmt.Errorf("At timeout not all pools are in errored state")
	}
	return nil
}
