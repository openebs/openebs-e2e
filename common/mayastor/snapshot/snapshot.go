...
func CreateVolumeSnapshot(snapshotClassName string, snapshotName string, pvc string, namespace string) (*v1.VolumeSnapshot, string, error) {
	// Get snapshot class before creating snapshot to verify snapshot class existence
	...
	if (err != nil && k8serrors.IsNotFound(err)) || vsc == nil {
		...
		// add sleep of five seconds before fetching snapshot class
...
func DeleteVolumeSnapshot(snapshotClassName string, snapshotName string, namespace string) error {
	...
	if err != nil && k8serrors.IsNotFound(err) {
		...
		return fmt.Errorf("failed to get snapshot class %s, error: %v", snapshotClassName, err)
	} else if vsc == nil {
		return fmt.Errorf("snapshot class %s not found, vsc: %v", snapshotClassName, *vsc)
	}
